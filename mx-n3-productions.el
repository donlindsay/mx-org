...
(defvar mx-n3-grammar-productions 'n3-bnf "BNF for N3") 	
(defvar mx-n3-tests '(setq notation-instance))

(defun mx-n3-symbol 	<foo#bar>    <http://example.com/> 	c d e f

(defun mx-n3-variable 	Any symbol quantified by @forAll or @forSome in the same or an outer formula. 	x y z

(defun mx-n3-formula 	{  ...  }  or an entire document 	F  G H K

(defun mx-n3-uni-var of F 	@forAll :x, :y. 	uvF

(defun mx-n3-ext-var of F 	@forSome :z, :w. 	evF

(defun mx-n3-statements of F 		stF

(defun mx-n3-statement 	  <#myCar>   <#color>   "green". 	Fi   or  {s p o}

(defun mx-n3-string 	"hello world" 	s

(defun mx-n3-integer 	34 	i

(defun mx-n3-list 	( 1 2 ?x  <a> ) 	L M

(defun mx-n3-element i of list L 		Li

(defun mx-n3-length-of-list 		|L|

(defun mx-n3-expression 	'see-grammar 	n m

(setq mx-n3-grammar-basic 
      '(($ 1, 2, <a> $)
	(S) 
	(T)))
